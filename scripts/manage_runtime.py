...
def detect_project_root() -> Path:
    ...
    # During development this script is copied to dist/scripts, while the project
    # root is one level above dist. In source form it lives in scripts directly.
...
CACHE = Path(os.environ.get('LABEL_STUDIO_RUNTIME_CACHE', str(ROOT / 'cache' / 'package' / 'pip'))).expanduser().resolve()
PIP_CACHE = Path(os.environ.get('PIP_CACHE_DIR', str(ROOT / 'cache' / 'package' / 'pip'))).expanduser().resolve()
ANACONDA_PKGS_MAIN_BASE_URL = 'https://repo.anaconda.com/pkgs/main/'
TUNA_ANACONDA_PKGS_MAIN_BASE_URL = 'https://mirrors.tuna.tsinghua.edu.cn/anaconda/pkgs/main/'
ANACONDA_MAIN_REPODATA = {}
...
def fetch_json_with_fallback(urls):
    return json.loads(fetch_text_with_fallback(urls))
...
def anaconda_runtime_platform_subdir():
...
def anaconda_main_repodata(subdir):
    if subdir not in ANACONDA_MAIN_REPODATA:
        ANACONDA_MAIN_REPODATA[subdir] = fetch_json_with_fallback([
            ANACONDA_PKGS_MAIN_BASE_URL + subdir + '/repodata.json',
            TUNA_ANACONDA_PKGS_MAIN_BASE_URL + subdir + '/repodata.json',
        ])
    return ANACONDA_MAIN_REPODATA[subdir]


def anaconda_python_packages():
    subdir = anaconda_runtime_platform_subdir()
    if not subdir:
        return []
    packages = anaconda_main_repodata(subdir).get('packages', {})
    candidates = []
    for filename, record in packages.items():
        if not filename.endswith('.tar.bz2'):
            continue
        if normalize_package_name(record.get('name')) != 'python':
            continue
        ...
        candidates.append({
            'filename': filename,
            'version': version,
            'build_number': int(record.get('build_number') or 0),
        })
    return candidates
...
def available_python_versions():
    if sys.platform != 'darwin':
        versions = {package['version'] for package in anaconda_python_packages()}
...
def python_package_name(version):
    if sys.platform != 'darwin':
        candidates = [
            package for package in anaconda_python_packages()
            if package['version'] == version
        ]
        candidates.sort(key=lambda item: item['build_number'], reverse=True)

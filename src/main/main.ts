import fs from 'node:fs';
import net from 'node:net';
import path from 'node:path';
import { app, BrowserWindow, clipboard, dialog, ipcMain, Menu, screen, session, type MessageBoxOptions, type MessageBoxReturnValue } from 'electron';
import { PythonService, PythonServiceError } from './services/pythonService';
import { RuntimeBootstrapService, BootstrapMode, windowsElectronInstallerPipePath } from './services/runtimeBootstrapService';
import { AppPaths } from './services/appPaths';
import { LaunchStage, LaunchStep, launchStage, launchStageWithTitleProgress, UpdatePreferenceDefaults, UpdatePreferenceKey, type LaunchProgressTitleStep } from './services/launchModels';
import { createSplashWindow, updateSplash, updateSplashDownloadPaused, waitForSplashRendererReady } from './windows/splashWindow';
import { UpdateWindowController } from './windows/updateWindow';
import { resolveAppIconPath } from './utils/appIcon';
import { initialWindowFrame, rememberWindowFrame } from './utils/windowFrameStore';
import {
    elevatedBootstrapModeFromArgv,
    elevatedBootstrapPipePathFromArgv,
    elevatedBootstrapUserDataPathFromArgv,
    elevatedBootstrapTokenFromArgv,
} from './utils/runtimeElevation';
import { runLinuxRuntimeReplacementWorker } from './utils/runtimeReplacementWorker';

const AppDisplayName = 'Label Studio';

class JsonStore {
    private data: Record<string, unknown> = {};

    constructor(private readonly file: string) {
        this.load();
        for (const [key, value] of Object.entries(UpdatePreferenceDefaults)) {
            if (!(key in this.data)) this.data[key] = value;
        }
        this.save();
    }

    load(): void {
        try {
            this.data = JSON.parse(fs.readFileSync(this.file, 'utf8')) as Record<string, unknown>;
        } catch {
            this.data = {};
        }
    }

    save(): void {
        fs.mkdirSync(path.dirname(this.file), { recursive: true });
        fs.writeFileSync(this.file, JSON.stringify(this.data, null, 2));
    }

    get(key: string): unknown {
        return this.data[key];
    }

    set(key: string, value: unknown): void {
        this.data[key] = value;
        this.save();
    }
}

type RuntimeVersionSnapshot = Awaited<ReturnType<RuntimeBootstrapService['fetchVersions']>>;

type RuntimeReadinessCache = {
    usable: boolean;
    versions?: RuntimeVersionSnapshot;
    updatedAt: number;
    reason: string;
    errorText?: string;
};

type LaunchProgressAnchor = {
    rawStart: number;
    rawEnd: number;
    displayStart: number;
    displayEnd: number;
};

class AppDelegate {
    private mainWindow?: BrowserWindow;
    private splashWindow?: BrowserWindow;
    private readonly pythonService = new PythonService();
    private readonly runtimeBootstrapService = new RuntimeBootstrapService();
    private readonly updateWindowController = new UpdateWindowController(
        this.runtimeBootstrapService,
        win => this.configureUpdateWindowMenu(win),
        () => this.hasMainWindowForRuntimeReload(),
        () => this.restartPythonServiceAndReloadMainWindow(),
        () => this.stopPythonServiceForRuntimeReplacement(),
        (mode, targetPackageVersion) => this.configureLaunchProgressTitlesForRuntimeBootstrapMode(mode, targetPackageVersion)
    );
    private mainApplicationMenu?: Menu;
    private updateApplicationMenu?: Menu;
    private splashApplicationMenu?: Menu;
    private isShuttingDown = false;

    private runtimeBootstrapInProgress = false;
    private runtimeBootstrapCompleted = false;
    private versionReadinessCache?: RuntimeReadinessCache;
    private versionReadinessRefreshPromise?: Promise<void>;
    private launchProgressTitleSteps: readonly LaunchProgressTitleStep[] = [];
    private launchProgressAnchor?: LaunchProgressAnchor;
    private currentLaunchProgress = 0;

    async applicationDidFinishLaunching(): Promise<void> {
        this.mainApplicationMenu = this.createApplicationMenu({ includeCheckForUpdates: true });
        this.updateApplicationMenu = this.createApplicationMenu({ includeCheckForUpdates: false });
        this.showSplashMenu();
        this.buildSplashWindow();

        this.runtimeBootstrapService.on('stage', stage => this.publishLaunchStage(stage));
        this.pythonService.on('stage', stage => this.publishLaunchStage(stage, stage.progress));

        try {
            await this.ensureRequiredRuntime();
            await this.checkProjectDataForLaunch();

            const autoElectron = Boolean(globalThis.sharedStore?.get(UpdatePreferenceKey.autoCheckElectron));
            const autoPackage = Boolean(globalThis.sharedStore?.get(UpdatePreferenceKey.autoCheckPackage));
            const autoPython = Boolean(globalThis.sharedStore?.get(UpdatePreferenceKey.autoCheckPython));
            if (autoElectron || autoPackage || autoPython) {
                this.publishLaunchStage(launchStage({
                    title: 'Checking Updates',
                    detail: 'Checking selected Electron, package, and runtime update preferences.',
                    progress: 0
                }), 0);
            }

            await this.updateWindowController.performAutomaticChecksIfNeeded();
            if (autoElectron || autoPackage || autoPython) {
                this.publishLaunchStage(launchStage({
                    title: 'Checking Updates',
                    detail: 'Selected update preferences are checked.',
                    progress: 1
                }), 1);
            }
            this.markVersionReadinessCacheDirty('after automatic checks');

            await this.openMainInterfaceWithRuntimeRollback();
            if (this.splashWindow && !this.splashWindow.isDestroyed()) this.splashWindow.close();
        } catch (error) {
            if (this.isShuttingDown) return;
            if (this.runtimeBootstrapService.hasPendingRuntimeReplacement()) {
                try {
                    await this.rollbackRuntimeAndOpenPreviousVersion(error);
                    if (this.splashWindow && !this.splashWindow.isDestroyed()) this.splashWindow.close();
                    return;
                } catch (recoveryError) {
                    const updateDetail = errorDetails(error);
                    const recoveryDetail = errorDetails(recoveryError);
                    await this.presentFailure(new Error(
                        `The startup update failed, and the previous managed runtime could not be restored and reopened.\n\nUpdate failure:\n${updateDetail}\n\nRecovery failure:\n${recoveryDetail}`,
                    ));
                    return;
                }
            }
            await this.presentFailure(error);
        }
    }

    private buildSplashWindow(): void {
        this.launchProgressAnchor = undefined;
        this.currentLaunchProgress = 0;
        this.splashWindow = createSplashWindow();
        this.configureSplashWindowMenu(this.splashWindow);

        ipcMain.on('launch-set-download-paused', (_event, paused: boolean) => {
            const requestedPaused = Boolean(paused);
            const actualPaused = this.runtimeBootstrapService.setCurrentDownloadPaused(requestedPaused);
            updateSplashDownloadPaused(this.splashWindow, actualPaused);
        });

        ipcMain.handle('launch-toggle-download-pause', () => {
            const paused = this.runtimeBootstrapService.toggleCurrentDownloadPause();
            updateSplashDownloadPaused(this.splashWindow, paused);
            return paused;
        });

        ipcMain.handle('launch-cancel-download', async () => {
            return await this.cancelCurrentDownloadFromSplash();
        });
    }

    private publishLaunchStage(stage: LaunchStage, fraction?: number): void {
        const mappedStage = launchStageWithTitleProgress(stage, fraction, this.launchProgressTitleSteps);
        const anchoredStage = this.applyLaunchProgressAnchor(mappedStage);
        const guardedStage = launchStage({
            ...anchoredStage,
            progress: Math.max(this.currentLaunchProgress, anchoredStage.progress),
        });
        this.currentLaunchProgress = guardedStage.progress;
        updateSplash(this.splashWindow, guardedStage);
    }

    private applyLaunchProgressAnchor(stage: LaunchStage): LaunchStage {
        const anchor = this.launchProgressAnchor;
        if (!anchor) return stage;

        const epsilon = 1e-6;
        if (stage.progress < anchor.rawStart - epsilon) return stage;
        if (stage.progress > anchor.rawEnd + epsilon) {
            this.launchProgressAnchor = undefined;
            return stage;
        }

        const rawFraction = (stage.progress - anchor.rawStart) / (anchor.rawEnd - anchor.rawStart);
        const progress = anchor.displayStart + (anchor.displayEnd - anchor.displayStart) * Math.max(0, Math.min(1, rawFraction));
        if (stage.progress >= anchor.rawEnd - epsilon) this.launchProgressAnchor = undefined;
        return launchStage({ ...stage, progress: Math.max(stage.progress, progress) });
    }

    private async checkProjectDataForLaunch(): Promise<void> {
        this.publishLaunchStage(launchStage({
            title: LaunchStep.migrations.title,
            detail: 'Verifying local runtime metadata and project data.',
            progress: 0
        }), 0);

        await this.refreshVersionReadinessCacheNow('launch project data check');

        this.publishLaunchStage(launchStage({
            title: LaunchStep.migrations.title,
            detail: 'Local runtime metadata and project data are ready.',
            progress: 1
        }), 1);
    }

    private async cancelCurrentDownloadFromSplash(): Promise<boolean> {
        const localPaused = this.runtimeBootstrapService.pauseCurrentDownload();
        if (localPaused) {
            updateSplashDownloadPaused(this.splashWindow, true);
        }

        const canOpenExistingRuntime = this.canOpenExistingRuntimeAfterCancelledDownload();
        let continueDownload: boolean;
        try {
            continueDownload = await this.showSplashCancelDownloadDialog(canOpenExistingRuntime);
        } catch (error) {
            this.runtimeBootstrapService.resumeCurrentDownload();
            updateSplashDownloadPaused(this.splashWindow, false);
            throw error;
        }

        if (continueDownload) {
            this.runtimeBootstrapService.resumeCurrentDownload();
            updateSplashDownloadPaused(this.splashWindow, false);
            return false;
        }

        this.runtimeBootstrapService.cancelCurrentDownloadAndSkip();
        updateSplashDownloadPaused(this.splashWindow, false);
        if (!canOpenExistingRuntime) {
            this.beginApplicationShutdown();
        }
        return true;
    }

    private async showSplashCancelDownloadDialog(canOpenExistingRuntime: boolean): Promise<boolean> {
        const result = await this.showMessageBox({
            type: 'question',
            message: canOpenExistingRuntime ? 'Stop Update?' : 'Cancel Download and Quit?',
            detail: canOpenExistingRuntime
                ? 'The current update download is paused. Continue downloading the update, or keep the currently installed version and open Label Studio now.'
                : 'The current download is paused. Label Studio cannot open until its required components have been downloaded.',
            buttons: canOpenExistingRuntime
                ? ['Continue Download', 'Open Label Studio']
                : ['Continue Download', 'Quit'],
            defaultId: 0,
            cancelId: 1
        });

        return result.response === 0;
    }

    private canOpenExistingRuntimeAfterCancelledDownload(): boolean {
        if (this.runtimeBootstrapInProgress && !this.runtimeBootstrapCompleted) return false;
        if (!AppPaths.isExecutable(AppPaths.bundledRuntimePython())) return false;
        return this.versionReadinessCache?.usable === true || this.bundledRuntimeHasPackageMetadata();
    }

    private async openMainInterfaceWithRuntimeRollback(): Promise<void> {
        try {
            await this.openMainInterfaceOnce();
            await this.runtimeBootstrapService.commitPendingRuntimeReplacement();
        } catch (error) {
            if (!this.runtimeBootstrapService.hasPendingRuntimeReplacement()) throw error;
            await this.rollbackRuntimeAndOpenPreviousVersion(error);
        }
    }

    private async rollbackRuntimeAndOpenPreviousVersion(updateError: unknown): Promise<void> {
        try { await this.pythonService.stopAndWait(); } catch { /* Continue with rollback after best-effort shutdown. */ }
        if (this.mainWindow && !this.mainWindow.isDestroyed()) this.mainWindow.destroy();
        this.mainWindow = undefined;
        await this.runtimeBootstrapService.rollbackPendingRuntimeReplacement();
        await this.openMainInterfaceOnce(true);
        await this.showMessageBox({
            type: 'warning',
            message: 'Update Rolled Back',
            detail: `The updated runtime or interface could not be opened, so Label Studio restored and reopened the previous runtime.\n\n${errorDetails(updateError)}`,
            buttons: ['OK'],
            defaultId: 0,
            cancelId: 0
        });
    }

    private async openMainInterfaceOnce(requireSuccessfulLoad = this.runtimeBootstrapService.hasPendingRuntimeReplacement()): Promise<void> {
        let baseURL: string;
        try {
            this.publishLaunchStage(launchStage({
                title: LaunchStep.localService.title,
                detail: 'Launching the local Label Studio web service.',
                progress: 0
            }), 0);
            baseURL = this.runtimeBootstrapService.hasPendingRuntimeReplacement()
                ? await this.pythonService.start()
                : await this.startPythonServiceWithRetry();
        } catch (error) {
            if (this.runtimeBootstrapService.hasPendingRuntimeReplacement()) throw error;
            if (!this.isRuntimeMissingError(error)) throw error;
            await this.ensureRequiredRuntime(true);
            this.markVersionReadinessCacheDirty('after forced runtime provisioning');
            await this.checkProjectDataForLaunch();
            this.publishLaunchStage(launchStage({
                title: LaunchStep.localService.title,
                detail: 'Launching the local Label Studio web service.',
                progress: 0
            }), 0);
            baseURL = this.runtimeBootstrapService.hasPendingRuntimeReplacement()
                ? await this.pythonService.start()
                : await this.startPythonServiceWithRetry();
        }

        this.publishLaunchStage(launchStage({
            title: LaunchStep.localService.title,
            detail: 'Local service is ready. Opening the workspace.',
            progress: 1
        }), 1);
        this.publishLaunchStage(LaunchStep.interfaceReady, 1);
        await this.buildMainWindow(baseURL, requireSuccessfulLoad || this.runtimeBootstrapService.hasPendingRuntimeReplacement());
    }

    private buildMainWindow(initialURL: string, requireSuccessfulLoad = false): Promise<void> {
        const display = this.splashWindow && !this.splashWindow.isDestroyed()
            ? screen.getDisplayMatching(this.splashWindow.getBounds())
            : screen.getPrimaryDisplay();
        const initialFrame = initialWindowFrame({
            id: 'main',
            display,
            defaultWidth: 1280,
            defaultHeight: 820,
            minWidth: 800,
            minHeight: 560
        });

        this.mainWindow = new BrowserWindow({
            ...initialFrame,
            minWidth: 800,
            minHeight: 560,
            title: 'Label Studio',
            show: false,
            backgroundColor: '#ffffff',
            icon: resolveAppIconPath(),
            webPreferences: { contextIsolation: true, nodeIntegration: false, sandbox: true }
        });
        this.configureMainWindowMenu(this.mainWindow);

        rememberWindowFrame(this.mainWindow, 'main');
        this.mainWindow.on('closed', () => { this.mainWindow = undefined; });

        return new Promise<void>((resolve, reject) => {
            let settled = false;
            const finish = (): void => {
                if (settled) return;
                settled = true;
                this.showMainMenu();
                this.mainWindow?.show();
                this.mainWindow?.focus();
                resolve();
            };
            const fail = (error: Error): void => {
                if (settled) return;
                settled = true;
                reject(error);
            };
            if (requireSuccessfulLoad) {
                this.mainWindow?.webContents.once('did-finish-load', finish);
                this.mainWindow?.webContents.once('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
                    if (isMainFrame === false) return;
                    fail(new Error(`The Label Studio interface failed to load (${errorCode}): ${errorDescription || validatedURL}`));
                });
                void this.mainWindow?.loadURL(initialURL).catch(error => {
                    fail(error instanceof Error ? error : new Error(String(error)));
                });
                return;
            }
            this.mainWindow?.once('ready-to-show', finish);
            this.mainWindow?.webContents.once('did-finish-load', finish);
            this.mainWindow?.webContents.once('did-fail-load', finish);
            setTimeout(finish, 2500);
            void this.mainWindow?.loadURL(initialURL);
        });
    }

    private async restartPythonServiceAndReloadMainWindow(): Promise<boolean> {
        const win = this.mainWindow;
        if (!win || win.isDestroyed()) return false;

        await this.stopPythonServiceForRuntimeReplacement();
        this.pythonService.resumeStartsAfterRuntimeReplacement();
        const baseURL = this.runtimeBootstrapService.hasPendingRuntimeReplacement()
            ? await this.pythonService.start()
            : await this.startPythonServiceWithRetry();

        if (win.isDestroyed()) return false;
        await win.loadURL(baseURL);
        win.show();
        win.focus();
        return true;
    }

    private async stopPythonServiceForRuntimeReplacement(): Promise<void> {
        const win = this.mainWindow;
        if (win && !win.isDestroyed()) win.webContents.stop();
        this.pythonService.suspendStartsForRuntimeReplacement();
        try {
            await this.stopPythonServiceWithRetry();
        } catch (error) {
            this.pythonService.resumeStartsAfterRuntimeReplacement();
            throw error;
        }
    }

    private async startPythonServiceWithRetry(): Promise<string> {
        while (true) {
            try {
                return await this.pythonService.start();
            } catch (error) {
                if (!this.isRetryablePythonServiceTimeout(error)) throw error;
                if (await this.presentFailure(error, true) !== 'retry') throw error;
                await this.stopPythonServiceWithRetry();
            }
        }
    }

    private async stopPythonServiceWithRetry(): Promise<void> {
        while (true) {
            try {
                await this.pythonService.stopAndWait();
                return;
            } catch (error) {
                if (!this.isRetryablePythonServiceTimeout(error)) throw error;
                if (await this.presentFailure(error, true) !== 'retry') throw error;
            }
        }
    }

    private isRetryablePythonServiceTimeout(error: unknown): error is PythonServiceError {
        return error instanceof PythonServiceError
            && (error.kind === 'startupTimedOut' || error.kind === 'stopTimedOut');
    }

    private hasMainWindowForRuntimeReload(): boolean {
        return Boolean(this.mainWindow && !this.mainWindow.isDestroyed());
    }

    private async showMessageBox(options: MessageBoxOptions): Promise<MessageBoxReturnValue> {
        const parent = this.splashWindow && !this.splashWindow.isDestroyed() ? this.splashWindow : undefined;
        await waitForSplashRendererReady(parent);
        const dialogOptions: MessageBoxOptions = { noLink: true, ...options };
        const response = parent ? await dialog.showMessageBox(parent, dialogOptions) : await dialog.showMessageBox(dialogOptions);
        return response;
    }

    private async presentFailure(error: unknown, allowRetry = false): Promise<'retry' | 'quit'> {
        const fullDetail = this.failureInformativeText(error);
        const cleanDetail = this.failureCleanDetailsFromText(fullDetail);
        const shortDetail = this.failureSummaryFromText(cleanDetail);

        let copiedNotice = '';

        const buttons = allowRetry
            ? ['Try Again', 'Copy Details and Quit', 'Only Quit']
            : ['Copy Details and Quit', 'Only Quit'];
        const copyDetailsResponse = allowRetry ? 1 : 0;

        const result = await this.showMessageBox({
            type: 'error',
            message: 'Failed to Start Label Studio',
            detail: [
                copiedNotice,
                shortDetail,
                allowRetry
                    ? 'Click "Try Again" to retry the Python service, or "Copy Details and Quit" to copy the error information.'
                    : 'Click "Copy Details and Quit" to copy the error information.'
            ].filter(Boolean).join('\n\n'),
            buttons,
            defaultId: allowRetry ? 0 : 1,
            cancelId: allowRetry ? 2 : 0,
            noLink: true
        });

        if (allowRetry && result.response === 0) return 'retry';

        if (result.response === copyDetailsResponse) {
            clipboard.writeText(cleanDetail);
            copiedNotice = 'Error information copied to clipboard.';
        }

        this.beginApplicationShutdown();
        return 'quit';
    }

    private failureCleanDetailsFromText(detail: string): string {
        const normalized = detail
            .replace(/\r\n/g, '\n')
            .replace(/\r/g, '\n');

        const cleanedLines: string[] = [];
        const seen = new Set<string>();

        for (const rawLine of normalized.split('\n')) {
            const cleanedLine = this.cleanFailureDetailLine(rawLine);

            if (cleanedLine == null) continue;

            const trimmed = cleanedLine.trim();

            if (trimmed.length === 0) {
                cleanedLines.push('');
                continue;
            }

            const key = this.failureCleanDedupeKey(trimmed);

            if (seen.has(key)) continue;
            seen.add(key);

            cleanedLines.push(cleanedLine);
        }

        const result = this.collapseBlankLines(cleanedLines).join('\n').trim();

        return result.length > 0 ? result : this.failureSummaryFromText(normalized);
    }

    private cleanFailureDetailLine(rawLine: string): string | undefined {
        const line = rawLine.trimEnd();
        const trimmed = line.trim();

        if (trimmed.length === 0) return '';

        // 把：
        // [2026-06-17 16:56:52,317] [urllib3.connectionpool::urlopen::869] [WARNING] Retrying ...
        // 变成：
        // [urllib3.connectionpool::urlopen::869] [WARNING] Retrying ...
        const timestampMatch = trimmed.match(/^\[\d{4}-\d{2}-\d{2}[^\]]*\]\s*(.+)$/);
        if (timestampMatch?.[1]) {
            return timestampMatch[1].trim();
        }

        return line;
    }

    private failureCleanDedupeKey(line: string): string {
        return line
            .replace(/\s+/g, ' ')
            // Retry(total=2/1/0) 本质上是同一类重复 warning，只保留第一次。
            .replace(/Retry\(total=\d+/g, 'Retry(total=*')
            .trim();
    }

    private collapseBlankLines(lines: string[]): string[] {
        const result: string[] = [];

        for (const line of lines) {
            const isBlank = line.trim().length === 0;
            const previousIsBlank = result.length > 0 && result[result.length - 1].trim().length === 0;

            if (isBlank && previousIsBlank) continue;
            result.push(line);
        }

        while (result.length > 0 && result[0].trim().length === 0) result.shift();
        while (result.length > 0 && result[result.length - 1].trim().length === 0) result.pop();

        return result;
    }

    private failureSummaryFromText(detail: string): string {
        const normalized = detail
            .replace(/\r\n/g, '\n')
            .replace(/\r/g, '\n');

        const tracebackError = this.extractLastTracebackFinalError(normalized);
        if (tracebackError) return tracebackError;

        const lines = normalized
            .split('\n')
            .map(line => line.trim())
            .filter(line => line.length > 0)
            .filter(line => !this.isFailureNoiseLine(line));

        for (let i = lines.length - 1; i >= 0; i -= 1) {
            const line = lines[i];

            if (this.looksLikeFailureExceptionLine(line)) {
                return this.compactFailureLine(line);
            }
        }

        for (let i = lines.length - 1; i >= 0; i -= 1) {
            const line = lines[i];

            if (this.looksLikeFailureKeywordLine(line)) {
                return this.compactFailureLine(line);
            }
        }

        for (let i = lines.length - 1; i >= 0; i -= 1) {
            const line = lines[i];
            const lower = line.toLowerCase();

            if (!lower.startsWith('file ')
                && !lower.startsWith('traceback ')
                && !lower.startsWith('during handling of the above exception')) {
                return this.compactFailureLine(line);
            }
        }

        return 'Unknown error.';
    }

    private extractLastTracebackFinalError(text: string): string | undefined {
        const lines = text.split('\n');

        let lastTracebackStart = -1;

        for (let i = 0; i < lines.length; i += 1) {
            if (lines[i].trim() === 'Traceback (most recent call last):') {
                lastTracebackStart = i;
            }
        }

        if (lastTracebackStart < 0) return undefined;

        const tracebackBlock: string[] = [];

        for (let i = lastTracebackStart; i < lines.length; i += 1) {
            const line = lines[i].trimEnd();

            if (i > lastTracebackStart && this.isFailureNoiseLine(line.trim())) {
                break;
            }

            tracebackBlock.push(line);
        }

        for (let i = tracebackBlock.length - 1; i >= 0; i -= 1) {
            const line = tracebackBlock[i].trim();
            const lower = line.toLowerCase();

            if (!line) continue;
            if (lower.startsWith('file ')) continue;
            if (lower.startsWith('traceback ')) continue;
            if (lower.startsWith('during handling of the above exception')) continue;
            if (this.isFailureNoiseLine(line)) continue;

            if (this.looksLikeFailureExceptionLine(line) || this.looksLikeFailureKeywordLine(line)) {
                return this.compactFailureLine(line);
            }
        }

        return undefined;
    }

    private looksLikeFailureExceptionLine(line: string): boolean {
        return /^((?:[A-Za-z_][\w]*\.)*[A-Za-z_][\w]*(?:Error|Exception|Timeout|Warning|error):\s*.+)$/i.test(line)
            || /^ERROR:\s*.+$/i.test(line);
    }

    private looksLikeFailureKeywordLine(line: string): boolean {
        return /\b(ENOENT|EACCES|ECONNREFUSED|ECONNRESET|ENOTFOUND|gaierror|ReadTimeoutError|NameResolutionError|ModuleNotFoundError|ImportError|TypeError|ReferenceError|SyntaxError|timed out|timeout|failed|not found|permission denied|nodename nor servname provided)\b/i.test(line);
    }

    private isFailureNoiseLine(line: string): boolean {
        const lower = line.toLowerCase();

        if (/^\[\d{4}-\d{2}-\d{2}[^\]]*\]\s*\[[^\]]+\]\s*\[(warning|info|debug)\]/i.test(line)) {
            return true;
        }

        if (/^\[\d{4}-\d{2}-\d{2}[^\]]*\]/i.test(line)
            && (lower.includes('[warning]') || lower.includes('[info]') || lower.includes('[debug]'))) {
            return true;
        }

        return lower.includes('urllib3.connectionpool')
            || lower.includes('[warning] retrying')
            || lower.includes('retrying (retry(')
            || lower.includes('sentry.io')
            || lower.includes('/api/5820521/envelope');
    }

    private compactFailureLine(line: string): string {
        const cleaned = line
            .replace(/^pip\._vendor\.urllib3\.exceptions\./, '')
            .replace(/^urllib3\.exceptions\./, '')
            .replace(/\s+/g, ' ')
            .trim();

        return cleaned.length > 900 ? `${cleaned.slice(0, 900).trimEnd()}…` : cleaned;
    }

    private failureInformativeText(error: unknown): string {
        const text = error instanceof Error ? (error.message || error.stack || String(error)) : String(error);
        const runtimeMatch = text.match(/Runtime bootstrap exited early with status (\d+)(?:[.:]\s*\n?\n?([\s\S]*))?/i)
            ?? text.match(/The embedded runtime command exited with status (\d+)(?:[.:]\s*\n?\n?([\s\S]*))?/i);
        if (runtimeMatch) {
            const status = runtimeMatch[1];
            const excerpt = (runtimeMatch[2] ?? '').trim();
            return excerpt.length > 0
                ? `Runtime bootstrap exited early with status ${status}.\n\n${excerpt}`
                : `Runtime bootstrap exited early with status ${status}.`;
        }

        const pythonMatch = text.match(/Python service exited early with status (\d+)(?:[.:]\s*\n?\n?([\s\S]*))?/i);
        if (pythonMatch) {
            const status = pythonMatch[1];
            const excerpt = (pythonMatch[2] ?? '').trim();
            return excerpt.length > 0
                ? `Python service exited early with status ${status}.\n\n${excerpt}`
                : `Python service exited early with status ${status}.`;
        }

        return error instanceof Error ? String(error) : text;
    }

    private async confirmBootstrapDownload(): Promise<boolean> {
        const result = await this.showMessageBox({
            type: 'question',
            message: 'Download Required Components?',
            detail: 'This app needs to download or update its embedded Python runtime, Label Studio package, and Electron runtime before it can open.\n\nDo you want to continue now?',
            buttons: ['Continue', 'Cancel'],
            defaultId: 0,
            cancelId: 1
        });
        return result.response === 0;
    }

    private async ensureRequiredRuntime(force = false): Promise<void> {
        const needsProvisioning = force || await this.needsBundledRuntimeProvisioning();
        await this.configureLaunchProgressTitles(needsProvisioning, force);

        if (!needsProvisioning) {
            this.refreshVersionReadinessCacheInBackground('runtime already provisioned');
            return;
        }

        this.runtimeBootstrapInProgress = true;
        this.runtimeBootstrapCompleted = false;
        this.versionReadinessCache = {
            usable: false,
            updatedAt: Date.now(),
            reason: 'bootstrap in progress'
        };

        try {
            await this.waitForVersionReadinessRefresh();
            await this.runtimeBootstrapService.waitForRuntimeReadersToSettle();

            if (!force) {
                const ok = await this.confirmBootstrapDownload();
                if (!ok) {
                    this.beginApplicationShutdown();
                    throw new Error('Runtime download was cancelled.');
                }
            }

            this.publishLaunchStage(launchStage({
                title: 'Preparing Download',
                detail: 'Starting the runtime and package download workflow.',
                progress: 0
            }), 0);

            await this.ensureRuntimeBootstrapWithMirrorFallback(BootstrapMode.ensurePackage);
            this.runtimeBootstrapCompleted = true;
            await this.refreshVersionReadinessCacheNow('bootstrap completed');
        } finally {
            this.runtimeBootstrapInProgress = false;
            this.refreshVersionReadinessCacheInBackground('bootstrap finished');
        }
    }

    private async ensureRuntimeBootstrapWithMirrorFallback(mode: BootstrapMode): Promise<void> {
        try {
            await this.runtimeBootstrapService.ensureRuntime(mode);
        } catch (error) {
            if (!this.runtimeBootstrapService.shouldRetryPyPIWithMirrorAfterFailure(error)) throw error;

            this.runtimeBootstrapService.forcePreferTunaPyPI('Retrying runtime bootstrap with the TUNA PyPI mirror after a primary network failure.');
            await this.runtimeBootstrapService.ensureRuntime(mode);
        }
    }

    private automaticUpdateChecksEnabled(): boolean {
        return Boolean(globalThis.sharedStore?.get(UpdatePreferenceKey.autoCheckElectron))
            || Boolean(globalThis.sharedStore?.get(UpdatePreferenceKey.autoCheckPackage))
            || Boolean(globalThis.sharedStore?.get(UpdatePreferenceKey.autoCheckPython));
    }

    private async configureLaunchProgressTitles(includeRuntimeBootstrap: boolean, forceRuntimeBootstrap: boolean): Promise<void> {
        this.launchProgressAnchor = undefined;
        const steps: LaunchProgressTitleStep[] = [];

        if (includeRuntimeBootstrap) {
            steps.push(['Preparing Download']);
            steps.push(...await this.runtimeBootstrapLaunchProgressTitles(forceRuntimeBootstrap));
        }

        steps.push(['Checking Project Data']);
        if (this.automaticUpdateChecksEnabled()) steps.push(['Checking Updates']);
        steps.push(['Starting Local Service']);
        steps.push(['Opening Interface']);

        this.launchProgressTitleSteps = this.deduplicateLaunchProgressTitles(steps);
    }

    private async runtimeBootstrapLaunchProgressTitles(forceRuntimeBootstrap: boolean): Promise<LaunchProgressTitleStep[]> {
        const hasRuntimePython = AppPaths.isExecutable(AppPaths.bundledRuntimePython());
        const runtimeValid = hasRuntimePython
            ? await this.runtimeBootstrapService.hasValidRuntime().catch(() => false)
            : false;
        const needsPythonRuntime = forceRuntimeBootstrap || !hasRuntimePython || !runtimeValid;
        const hasUsablePackage = !needsPythonRuntime
            && await this.runtimeBootstrapService.hasUsablePackage().catch(() => false);
        const needsPackage = forceRuntimeBootstrap || needsPythonRuntime || !hasUsablePackage;
        const cacheTitle = AppPaths.shouldReclaimRuntimeCache() ? 'Reclaiming Cache' : 'Cache Preserved';

        const steps: LaunchProgressTitleStep[] = [
            ['Preparing Runtime']
        ];

        if (needsPythonRuntime) {
            steps.push(
                ['Downloading Python', 'Runtime Download Skipped'],
                ['Expanding Runtime'],
                ['Runtime Expanded'],
                ['Installing Runtime'],
                ['Runtime Installed']
            );
        }

        if (needsPackage) {
            steps.push(
                ['Preparing Package'],
                ['Bootstrapping pip'],
                ['Pip Ready'],
                ['Installing', 'Installing Label Studio'],
                ['Label Studio Installed'],
                ['Optimizing Runtime'],
                ['Runtime Optimized'],
                ['Applying Runtime']
            );
        }

        steps.push(
            ['Checking Electron', 'Updating Electron'],
            ['Electron Ready'],
            [cacheTitle],
            ['Runtime Ready']
        );

        return steps;
    }

    private async configureLaunchProgressTitlesForRuntimeBootstrapMode(
        mode: BootstrapMode,
        targetPackageVersion?: string
    ): Promise<void> {
        if (!this.splashWindow || this.splashWindow.isDestroyed()) return;

        const runtimeSteps = await this.runtimeBootstrapLaunchProgressTitlesForMode(mode, targetPackageVersion);
        const steps: LaunchProgressTitleStep[] = [
            ['Checking Project Data'],
            ['Checking Updates'],
            ...runtimeSteps,
            ['Starting Local Service'],
            ['Opening Interface']
        ];
        this.launchProgressTitleSteps = this.deduplicateLaunchProgressTitles(steps);
        this.launchProgressAnchor = this.launchProgressAnchorForRuntimeSteps(this.launchProgressTitleSteps, runtimeSteps);
    }

    private async runtimeBootstrapLaunchProgressTitlesForMode(
        mode: BootstrapMode,
        targetPackageVersion?: string
    ): Promise<LaunchProgressTitleStep[]> {
        switch (mode) {
            case BootstrapMode.updateElectron:
                return this.electronUpdateLaunchProgressTitles();
            case BootstrapMode.ensureElectron:
                return this.electronCheckLaunchProgressTitles();
            case BootstrapMode.updatePackage:
                return this.packageUpdateLaunchProgressTitles(
                    await this.runtimeBootstrapService.packageUpdateRequiresRuntimeReplacement(targetPackageVersion).catch(() => true)
                );
            case BootstrapMode.updatePython:
            case BootstrapMode.updateAll:
                return this.pythonAndPackageUpdateLaunchProgressTitles(mode === BootstrapMode.updateAll);
            case BootstrapMode.ensureAll:
            case BootstrapMode.ensurePackage:
            default:
                return await this.runtimeBootstrapLaunchProgressTitles(false);
        }
    }

    private packageUpdateLaunchProgressTitles(includePythonRuntime: boolean): LaunchProgressTitleStep[] {
        return [
            ['Preparing Runtime'],
            ...(includePythonRuntime ? this.pythonRuntimeLaunchProgressTitles() : []),
            ...this.packageRuntimeLaunchProgressTitles(),
            ...this.runtimeBootstrapCompletionLaunchProgressTitles()
        ];
    }

    private pythonAndPackageUpdateLaunchProgressTitles(includeElectronCheck: boolean): LaunchProgressTitleStep[] {
        return [
            ['Preparing Runtime'],
            ...this.pythonRuntimeLaunchProgressTitles(),
            ...this.packageRuntimeLaunchProgressTitles(),
            ...(includeElectronCheck ? this.electronCheckLaunchProgressTitles(false) : []),
            ...this.runtimeBootstrapCompletionLaunchProgressTitles()
        ];
    }

    private pythonRuntimeLaunchProgressTitles(): LaunchProgressTitleStep[] {
        return [
            ['Downloading Python', 'Runtime Download Skipped'],
            ['Expanding Runtime'],
            ['Runtime Expanded'],
            ['Installing Runtime'],
            ['Runtime Installed']
        ];
    }

    private packageRuntimeLaunchProgressTitles(): LaunchProgressTitleStep[] {
        return [
            ['Preparing Package'],
            ['Bootstrapping pip'],
            ['Pip Ready'],
            ['Installing', 'Installing Label Studio'],
            ['Label Studio Installed'],
            ['Optimizing Runtime'],
            ['Runtime Optimized'],
            ['Applying Runtime']
        ];
    }

    private electronCheckLaunchProgressTitles(includeCompletion = true): LaunchProgressTitleStep[] {
        return [
            ['Preparing Runtime'],
            ['Checking Electron', 'Updating Electron'],
            ['Electron Ready'],
            ...(includeCompletion ? this.runtimeBootstrapCompletionLaunchProgressTitles() : [])
        ];
    }

    private electronUpdateLaunchProgressTitles(): LaunchProgressTitleStep[] {
        return [
            ['Preparing Runtime'],
            ['Downloading Electron'],
            ['Expanding Electron'],
            ['Installing Electron'],
            ['Electron Updated', 'Electron Update Ready'],
            ...this.runtimeBootstrapCompletionLaunchProgressTitles()
        ];
    }

    private runtimeBootstrapCompletionLaunchProgressTitles(): LaunchProgressTitleStep[] {
        const cacheTitle = AppPaths.shouldReclaimRuntimeCache() ? 'Reclaiming Cache' : 'Cache Preserved';
        return [
            [cacheTitle],
            ['Runtime Ready']
        ];
    }

    private launchProgressAnchorForRuntimeSteps(
        allSteps: readonly LaunchProgressTitleStep[],
        runtimeSteps: readonly LaunchProgressTitleStep[]
    ): LaunchProgressAnchor | undefined {
        const firstRuntimeStep = runtimeSteps[0];
        const lastRuntimeStep = runtimeSteps[runtimeSteps.length - 1];
        if (!firstRuntimeStep || !lastRuntimeStep || allSteps.length === 0) return undefined;

        const firstIndex = this.launchProgressStepIndex(allSteps, firstRuntimeStep);
        const lastIndex = this.launchProgressStepIndex(allSteps, lastRuntimeStep);
        if (firstIndex < 0 || lastIndex < firstIndex) return undefined;

        const rawStart = firstIndex / allSteps.length;
        const rawEnd = (lastIndex + 1) / allSteps.length;
        const displayStart = this.currentLaunchProgress;
        if (displayStart <= rawStart + 1e-6) return undefined;

        const displayEnd = rawEnd > displayStart + 1e-6
            ? rawEnd
            : this.nextLaunchProgressBoundaryAfter(allSteps, lastIndex + 1, displayStart) ?? 1;
        if (displayEnd <= displayStart + 1e-6 || rawEnd <= rawStart) return undefined;
        return { rawStart, rawEnd, displayStart, displayEnd };
    }

    private launchProgressStepIndex(
        allSteps: readonly LaunchProgressTitleStep[],
        step: LaunchProgressTitleStep
    ): number {
        const key = step.join('\u0000');
        return allSteps.findIndex(candidate => candidate.join('\u0000') === key);
    }

    private nextLaunchProgressBoundaryAfter(
        steps: readonly LaunchProgressTitleStep[],
        startIndex: number,
        progress: number
    ): number | undefined {
        for (let index = Math.max(0, startIndex); index <= steps.length; index += 1) {
            const boundary = index / steps.length;
            if (boundary > progress + 1e-6) return boundary;
        }
        return undefined;
    }

    private deduplicateLaunchProgressTitles(steps: LaunchProgressTitleStep[]): readonly LaunchProgressTitleStep[] {
        const seen = new Set<string>();
        return steps.filter(step => {
            const key = step.join('\u0000');
            if (seen.has(key)) return false;
            seen.add(key);
            return true;
        });
    }

    private isRuntimeMissingError(error: unknown): boolean {
        const text = error instanceof Error ? `${error.message}\n${error.stack ?? ''}` : String(error);
        return text.includes('Missing runtime')
            || text.includes('ENOENT')
            || text.includes('PythonCore')
            || text.includes('/bin/Python')
            || text.includes("No module named 'label_studio'")
            || text.includes('does not provide an importable label_studio.server:main entry point');
    }

    private async needsBundledRuntimeProvisioning(): Promise<boolean> {
        if (!AppPaths.isExecutable(AppPaths.bundledRuntimePython())) {
            return true;
        }

        if (!this.runtimeBootstrapService.hasUsableElectronRuntime()) {
            return true;
        }

        if (!(await this.runtimeBootstrapService.hasValidRuntime())) {
            return true;
        }

        if (!(await this.runtimeBootstrapService.hasUsablePackage())) {
            return true;
        }

        if (this.versionReadinessCache?.usable === true) {
            return false;
        }

        if (this.versionReadinessCache?.usable === false && this.versionReadinessCache.reason.includes('missing')) {
            return true;
        }

        this.refreshVersionReadinessCacheInBackground('needsBundledRuntimeProvisioning cache miss');
        return false;
    }

    private async hasUsableEmbeddedRuntimeAndPackage(allowDuringBootstrap = false): Promise<boolean> {
        const bundledRuntimePython = AppPaths.bundledRuntimePython();

        if (!AppPaths.isExecutable(bundledRuntimePython)) {
            this.versionReadinessCache = {
                usable: false,
                updatedAt: Date.now(),
                reason: 'bundled Python missing'
            };
            return false;
        }

        if (!allowDuringBootstrap && this.runtimeBootstrapInProgress && !this.runtimeBootstrapCompleted) {
            return false;
        }

        this.refreshVersionReadinessCacheInBackground('hasUsableEmbeddedRuntimeAndPackage');

        if (this.versionReadinessCache?.usable === true) return true;
        return await this.runtimeBootstrapService.hasUsablePackage().catch(() => false);
    }

    private bundledRuntimeHasPackageMetadata(): boolean {
        for (const sitePackages of this.sitePackagesDirectories(AppPaths.bundledRuntimeRoot())) {
            if (!fs.existsSync(sitePackages)) continue;

            let entries: string[];
            try {
                entries = fs.readdirSync(sitePackages);
            } catch {
                continue;
            }

            for (const entry of entries) {
                if (!/^label[-_.]studio-[^-]+\.dist-info$/i.test(entry)) continue;
                if (this.packageMetadataIsLabelStudio(path.join(sitePackages, entry, 'METADATA'))) return true;
            }
        }

        return false;
    }

    private packageMetadataIsLabelStudio(metadataPath: string): boolean {
        try {
            const text = fs.readFileSync(metadataPath, 'utf8');
            const name = text.match(/^Name:\s*(.+)$/mi)?.[1]?.trim().toLowerCase();
            const version = text.match(/^Version:\s*(.+)$/mi)?.[1]?.trim();
            return name === 'label-studio' && Boolean(version);
        } catch {
            return false;
        }
    }

    private sitePackagesDirectories(runtimeRoot: string): string[] {
        const directories = new Set<string>();
        directories.add(path.join(runtimeRoot, 'Lib', 'site-packages'));

        const libRoot = path.join(runtimeRoot, 'lib');
        for (const pythonDirectory of this.childDirectoriesMatching(libRoot, /^python\d+(?:\.\d+)?$/)) {
            directories.add(path.join(pythonDirectory, 'site-packages'));
        }

        const frameworkVersions = path.join(runtimeRoot, 'Library', 'Frameworks', 'Python.framework', 'Versions');
        for (const versionDirectory of this.childDirectoriesMatching(frameworkVersions, /^(?:Current|\d+(?:\.\d+)*)$/)) {
            const frameworkLib = path.join(versionDirectory, 'lib');
            for (const pythonDirectory of this.childDirectoriesMatching(frameworkLib, /^python\d+(?:\.\d+)?$/)) {
                directories.add(path.join(pythonDirectory, 'site-packages'));
            }
        }

        return [...directories];
    }

    private childDirectoriesMatching(parent: string, pattern: RegExp): string[] {
        try {
            return fs.readdirSync(parent, { withFileTypes: true })
                .filter(entry => entry.isDirectory() || entry.isSymbolicLink())
                .filter(entry => pattern.test(entry.name))
                .map(entry => path.join(parent, entry.name));
        } catch {
            return [];
        }
    }

    private markVersionReadinessCacheDirty(reason: string): void {
        this.versionReadinessCache = undefined;
        this.refreshVersionReadinessCacheInBackground(reason);
    }

    private refreshVersionReadinessCacheInBackground(reason: string): void {
        if (this.runtimeBootstrapInProgress) return;
        if (this.versionReadinessRefreshPromise) return;

        this.versionReadinessRefreshPromise = this.refreshVersionReadinessCacheNow(reason)
            .catch(error => {
                this.versionReadinessCache = {
                    usable: false,
                    updatedAt: Date.now(),
                    reason,
                    errorText: this.errorText(error)
                };
            })
            .finally(() => {
                this.versionReadinessRefreshPromise = undefined;
            });
    }

    private async waitForVersionReadinessRefresh(): Promise<void> {
        const pending = this.versionReadinessRefreshPromise;
        if (!pending) return;
        await pending.catch(() => undefined);
    }

    private async refreshVersionReadinessCacheNow(reason: string): Promise<void> {
        if (this.runtimeBootstrapInProgress && !this.runtimeBootstrapCompleted) {
            this.versionReadinessCache = {
                usable: false,
                updatedAt: Date.now(),
                reason: `${reason}: bootstrap in progress`
            };
            return;
        }

        const bundledRuntimePython = AppPaths.bundledRuntimePython();
        if (!AppPaths.isExecutable(bundledRuntimePython)) {
            this.versionReadinessCache = {
                usable: false,
                updatedAt: Date.now(),
                reason: `${reason}: bundled Python missing`
            };
            return;
        }

        const [versions, packageUsable] = await Promise.all([
            this.runtimeBootstrapService.fetchVersions(),
            this.runtimeBootstrapService.hasUsablePackage(),
        ]);
        this.updateVersionReadinessCacheFromVersions(versions, reason, packageUsable);
    }

    private updateVersionReadinessCacheFromVersions(
        versions: RuntimeVersionSnapshot,
        reason: string,
        packageUsable: boolean,
    ): void {
        const hasPackage = Boolean(versions.packageVersion)
            && versions.packageVersion !== 'Not installed'
            && versions.packageVersion !== 'Unknown';

        const hasPython = Boolean(versions.pythonVersion)
            && versions.pythonVersion !== 'Not installed'
            && versions.pythonVersion !== 'Unknown';

        this.versionReadinessCache = {
            usable: hasPackage && packageUsable && hasPython && this.runtimeBootstrapService.hasUsableElectronRuntime(),
            versions,
            updatedAt: Date.now(),
            reason
        };
    }

    private errorText(error: unknown): string {
        return error instanceof Error ? (error.message || error.stack || String(error)) : String(error);
    }

    private configureSplashWindowMenu(win: BrowserWindow): void {
        if (process.platform !== 'darwin') {
            win.setAutoHideMenuBar(true);
            win.setMenuBarVisibility(false);
            win.setMenu(null);
        }
        win.on('focus', () => this.showSplashMenu());
        win.on('show', () => {
            if (win.isFocused()) this.showSplashMenu();
        });
        win.on('closed', () => {
            if (this.mainWindow && !this.mainWindow.isDestroyed() && this.mainWindow.isFocused()) {
                this.showMainMenu();
            }
        });
    }

    private configureUpdateWindowMenu(win: BrowserWindow): void {
        if (process.platform !== 'darwin') {
            win.setAutoHideMenuBar(true);
            win.setMenuBarVisibility(false);
            win.setMenu(null);
        }
        win.on('focus', () => this.showUpdateMenu());
        win.on('show', () => {
            if (win.isFocused()) this.showUpdateMenu();
        });
        win.on('closed', () => {
            if (this.mainWindow && !this.mainWindow.isDestroyed() && this.mainWindow.isFocused()) {
                this.showMainMenu();
            }
        });
    }

    private configureMainWindowMenu(win: BrowserWindow): void {
        win.setAutoHideMenuBar(false);
        win.setMenuBarVisibility(true);
        if (process.platform !== 'darwin') win.setMenu(this.mainApplicationMenu ?? this.createApplicationMenu({ includeCheckForUpdates: true }));
        win.on('focus', () => this.showMainMenu());
        win.on('show', () => {
            if (win.isFocused()) this.showMainMenu();
        });
    }

    private showMainMenu(): void {
        this.mainApplicationMenu ??= this.createApplicationMenu({ includeCheckForUpdates: true });
        if (process.platform === 'darwin') {
            Menu.setApplicationMenu(this.mainApplicationMenu);
        } else if (this.mainWindow && !this.mainWindow.isDestroyed()) {
            this.mainWindow.setMenu(this.mainApplicationMenu);
        }
    }

    private showUpdateMenu(): void {
        if (process.platform !== 'darwin') return;
        this.updateApplicationMenu ??= this.createApplicationMenu({ includeCheckForUpdates: false });
        Menu.setApplicationMenu(this.updateApplicationMenu);
    }

    private showSplashMenu(): void {
        if (process.platform !== 'darwin') {
            Menu.setApplicationMenu(null);
            return;
        }

        this.splashApplicationMenu ??= this.createSplashMenu();
        Menu.setApplicationMenu(this.splashApplicationMenu);
    }

    private createSplashMenu(): Menu {
        const appName = AppDisplayName;
        return Menu.buildFromTemplate([
            {
                label: appName,
                submenu: [
                    { role: 'about', label: `About ${appName}` },
                    { type: 'separator' },
                    { role: 'services' },
                    { type: 'separator' },
                    { label: `Hide ${appName}`, accelerator: 'Command+H', role: 'hide' },
                    { label: 'Hide Others', accelerator: 'Command+Alt+H', role: 'hideOthers' },
                    { label: 'Show All', role: 'unhide' },
                    { type: 'separator' },
                    { label: `Quit ${appName}`, accelerator: 'Command+Q', role: 'quit' },
                ],
            },
        ]);
    }

    private createApplicationMenu(options: { includeCheckForUpdates: boolean }): Menu {
        const appName = AppDisplayName;
        const web = (): Electron.WebContents | undefined => {
            const focusedWindow = BrowserWindow.getFocusedWindow();
            if (focusedWindow && !focusedWindow.isDestroyed()) return focusedWindow.webContents;
            if (!this.mainWindow || this.mainWindow.isDestroyed()) return undefined;
            return this.mainWindow.webContents;
        };
        const edit = (name: 'undo' | 'redo' | 'cut' | 'copy' | 'paste' | 'delete' | 'selectAll'): void => {
            const contents = web() as (Electron.WebContents & Record<string, () => void>) | undefined;
            contents?.[name]?.();
        };
        const adjustZoom = (delta: number): void => {
            const contents = web();
            if (!contents) return;
            contents.setZoomLevel(contents.getZoomLevel() + delta);
        };

        const appSubmenu: Electron.MenuItemConstructorOptions[] = [
            ...(options.includeCheckForUpdates
                ? [
                    { label: 'Check for Updates', accelerator: 'CommandOrControl+U', click: () => this.updateWindowController.presentWindow() } satisfies Electron.MenuItemConstructorOptions,
                    { type: 'separator' } satisfies Electron.MenuItemConstructorOptions
                ]
                : []),
            { label: 'Clear HTTP Cache', click: () => { void this.clearHTTPCache(); } },
            { label: 'Clear Cookies', click: () => { void this.clearCookies(); } },
            { type: 'separator' },
            { label: `Hide ${appName}`, accelerator: 'Command+H', role: 'hide' },
            { label: 'Hide Others', accelerator: 'Command+Alt+H', role: 'hideOthers' },
            { label: 'Show All', role: 'unhide' },
            { type: 'separator' },
            { label: `Quit ${appName}`, accelerator: 'Command+Q', role: 'quit' },
        ];

        const template: Electron.MenuItemConstructorOptions[] = [
            {
                label: appName,
                submenu: appSubmenu,
            },
            {
                label: 'Edit',
                submenu: [
                    { label: 'Undo', accelerator: 'CommandOrControl+Z', click: () => edit('undo') },
                    { label: 'Redo', accelerator: 'CommandOrControl+Shift+Z', click: () => edit('redo') },
                    { type: 'separator' },
                    { label: 'Cut', accelerator: 'CommandOrControl+X', click: () => edit('cut') },
                    { label: 'Copy', accelerator: 'CommandOrControl+C', click: () => edit('copy') },
                    { label: 'Paste', accelerator: 'CommandOrControl+V', click: () => edit('paste') },
                    { label: 'Delete', accelerator: process.platform === 'darwin' ? 'Command+Backspace' : 'Control+Backspace', click: () => edit('delete') },
                    { type: 'separator' },
                    { label: 'Select All', accelerator: 'CommandOrControl+A', click: () => edit('selectAll') },
                ],
            },
            {
                label: 'Navigation',
                submenu: [
                    { label: 'Back', accelerator: 'CommandOrControl+[', click: () => { const c = web(); if (c?.canGoBack()) c.goBack(); } },
                    { label: 'Forward', accelerator: 'CommandOrControl+]', click: () => { const c = web(); if (c?.canGoForward()) c.goForward(); } },
                    { type: 'separator' },
                    { label: 'Reload', accelerator: 'CommandOrControl+R', click: () => web()?.reload() },
                    { label: 'Reload Ignoring Cache', accelerator: 'CommandOrControl+Shift+R', click: () => web()?.reloadIgnoringCache() },
                    { label: 'Stop', accelerator: 'CommandOrControl+.', click: () => web()?.stop() },
                ],
            },
            {
                label: 'View',
                submenu: [
                    { label: 'Zoom In', accelerator: process.platform === 'darwin' ? 'Command+Plus' : 'Control+Plus', click: () => adjustZoom(1) },
                    { label: 'Zoom Out', accelerator: 'CommandOrControl+-', click: () => adjustZoom(-1) },
                    { label: 'Actual Size', accelerator: 'CommandOrControl+0', click: () => web()?.setZoomLevel(0) },
                    { type: 'separator' },
                    { label: 'Enter Full Screen', accelerator: process.platform === 'darwin' ? 'Ctrl+Command+F' : 'F11', click: () => this.mainWindow?.setFullScreen(!this.mainWindow.isFullScreen()) },
                    { type: 'separator' },
                    { label: 'Toggle Developer Tools', accelerator: process.platform === 'darwin' ? 'Alt+Command+I' : 'Ctrl+Shift+I', click: () => web()?.toggleDevTools() },
                ],
            },
        ];
        return Menu.buildFromTemplate(template);
    }

    private async clearHTTPCache(): Promise<void> {
        const targetSession = this.mainWindow && !this.mainWindow.isDestroyed()
            ? this.mainWindow.webContents.session
            : session.defaultSession;
        await targetSession.clearCache();
    }

    private async clearCookies(): Promise<void> {
        const targetSession = this.mainWindow && !this.mainWindow.isDestroyed()
            ? this.mainWindow.webContents.session
            : session.defaultSession;
        await targetSession.clearStorageData({ storages: ['cookies'] });
    }

    beginApplicationShutdown(): void {
        if (this.isShuttingDown) return;
        this.isShuttingDown = true;
        this.stop();
        app.quit();
    }

    prepareApplicationQuit(): void {
        this.isShuttingDown = true;
        this.stop();
    }

    focusMainWindow(): void {
        if (!this.mainWindow) return;
        if (this.mainWindow.isMinimized()) this.mainWindow.restore();
        this.mainWindow.focus();
    }

    stop(): void {
        this.pythonService.stop();
        this.runtimeBootstrapService.stop();
        if (this.splashWindow && !this.splashWindow.isDestroyed()) {
            this.splashWindow.close();
            this.splashWindow = undefined;
        }
        if (this.mainWindow && !this.mainWindow.isDestroyed()) {
            this.mainWindow.close();
            this.mainWindow = undefined;
        }
    }
}

function appendElectronUpdateLog(message: string): void {
    try {
        fs.appendFileSync(AppPaths.electronUpdateApplyLogFile(), `${new Date().toISOString()} ${message}\n`, 'utf8');
    } catch {
        // Startup protection must never fail because logging is unavailable.
    }
}

async function windowsElectronInstallerIsActive(): Promise<boolean> {
    if (process.platform !== 'win32') return false;

    return await new Promise<boolean>((resolve) => {
        const socket = net.createConnection(windowsElectronInstallerPipePath(app.getPath('exe')));
        let settled = false;
        let response = '';

        const finish = (active: boolean): void => {
            if (settled) return;
            settled = true;
            socket.destroy();
            resolve(active);
        };

        socket.setEncoding('utf8');
        socket.once('connect', () => socket.write('ACTIVATE\n'));
        socket.on('data', chunk => {
            response += chunk;
            const newline = response.indexOf('\n');
            if (newline >= 0) finish(response.slice(0, newline).trim() === 'ACTIVE');
        });
        socket.once('error', () => finish(false));
        socket.once('end', () => finish(response.trim() === 'ACTIVE'));
    });
}

async function quitIfWindowsElectronInstallerIsActive(): Promise<boolean> {
    if (!await windowsElectronInstallerIsActive()) return false;
    appendElectronUpdateLog('Startup handed back to the active Windows Electron installer through direct IPC.');
    app.exit(0);
    return true;
}

process.title = AppDisplayName;
app.setName(AppDisplayName);
if (process.platform === 'win32') {
    app.setAppUserModelId(AppDisplayName);
} else if (process.platform === 'darwin') {
    const dockIconPath = resolveAppIconPath();
    try {
        if (dockIconPath && app.dock) app.dock.setIcon(dockIconPath);
    } catch (error) {
        console.warn('[app] failed to set dock icon:', error);
    }
}

const elevatedBootstrapMode = elevatedBootstrapModeFromArgv(process.argv);
const elevatedBootstrapUserDataPath = elevatedBootstrapUserDataPathFromArgv(process.argv);
app.setPath('userData', elevatedBootstrapUserDataPath ?? path.join(app.getPath('appData'), AppPaths.getAppSupportDirectoryName()));
let delegate: AppDelegate | undefined;

if (elevatedBootstrapMode) {
    app.whenReady().then(async () => {
        await runLinuxRuntimeReplacementWorker(
            elevatedBootstrapPipePathFromArgv(process.argv),
            elevatedBootstrapTokenFromArgv(process.argv),
        );
        app.exit(0);
    }).catch(error => {
        appendRuntimeBootstrapLog(`Runtime file replacement failed:\n${errorDetails(error)}`);
        app.exit(1);
    });
} else {
    const gotLock = app.requestSingleInstanceLock();

    if (!gotLock) {
        app.quit();
    } else {
        app.on('second-instance', () => delegate?.focusMainWindow());
        app.on('activate', () => delegate?.focusMainWindow());
        app.whenReady().then(async () => {
            app.setAccessibilitySupportEnabled(true);
            if (await quitIfWindowsElectronInstallerIsActive()) return;
            globalThis.sharedStore = new JsonStore(path.join(app.getPath('userData'), 'preferences.json'));
            delegate = new AppDelegate();
            await delegate.applicationDidFinishLaunching();
        }).catch(error => {
            console.error(error);
            app.quit();
        });
        app.on('window-all-closed', () => {
            if (process.platform === 'win32') appendElectronUpdateLog('[lifecycle] window-all-closed received; calling app.quit().');
            app.quit();
        });
        app.on('before-quit', () => {
            if (process.platform === 'win32') appendElectronUpdateLog('[lifecycle] before-quit received.');
            delegate?.prepareApplicationQuit();
        });
        app.on('will-quit', () => {
            if (process.platform === 'win32') appendElectronUpdateLog('[lifecycle] will-quit received.');
        });
    }
}

process.once('exit', code => {
    if (process.platform === 'win32') appendElectronUpdateLog(`[lifecycle] Node process exit. code=${code}`);
});

function appendRuntimeBootstrapLog(message: string): void {
    try {
        fs.mkdirSync(path.dirname(AppPaths.runtimeBootstrapLogFile()), { recursive: true });
        fs.appendFileSync(AppPaths.runtimeBootstrapLogFile(), `${new Date().toISOString()} ${message}\n`, 'utf8');
    } catch {
        // Bootstrap logging must not hide the original startup/install error.
    }
}

function errorDetails(error: unknown): string {
    if (error instanceof Error) return error.stack || error.message;
    return String(error);
}

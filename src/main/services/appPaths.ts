...
export class AppPaths {
    ...
    private static readonly appSupportDirectoryName = path.join("label-studio", "client");

    static getAppSupportDirectoryName(): string {
        // return process.platform === "darwin"
        //   ? this.macAppSupportDirectoryName
        //   : this.portableAppSupportDirectoryName;
        return this.appSupportDirectoryName;
    }
    ...
    static resourcesRoot(): string {
        ...
        // Development mode copies assets and scripts into dist for build, but source root
        // also contains those resources. Using projectRoot keeps behavior simple and
    ...
    static bundledRuntimeRoot(): string {
        ...
        if (app.isPackaged) {
            ...
            //   macOS:   Label Studio.app/Contents/Resources/Runtime
            //   Windows: <install dir>/resources/runtime
            //   Linux:   <install dir>/resources/runtime
            return path.join(process.resourcesPath, this.runtimeDirectoryName());
        }
    ...
    static launcherScript(): string {
        return this.firstExistingResourceFile("launch_label_studio.py", this.scriptResourceDirectoryNames());
    }

    static runtimeManagerScript(): string {
        return this.firstExistingResourceFile("manage_runtime.py", this.scriptResourceDirectoryNames());
    }
    ...
    private static scriptResourceDirectoryNames(): string[] {
        return process.platform === "darwin" ? ["Scripts", "scripts"] : ["scripts", "Scripts"];
    }
    ...
    static windowsInstallerExecutable(): string {
        if (process.platform !== "win32") {
            throw new Error("The Label Studio installer executable is only available on Windows.");
        }
        if (process.arch !== "x64" && process.arch !== "arm64" && process.arch !== "ia32") {
            throw new Error(`The Label Studio installer does not support Windows architecture ${process.arch}.`);
        }
        return app.isPackaged
            ? path.join(process.resourcesPath, "installer", "Label Studio Installer.exe")
            : path.join(this.projectRoot(), "dist", "windows-installer", process.arch, "Label Studio Installer.exe");
    }

    static electronUpdateApplyLogFile(): string {
        return path.join(app.getPath("userData"), "cache", "electron", "electron-update-apply.log");
    }

    static runtimeBootstrapLogFile(): string {
        return path.join(app.getPath("userData"), "cache", "runtime-bootstrap.log");
    }
    ...
    static packageCondaForgeCacheDirectory(): string {
        return this.ensureDirectory(path.join(this.packageDownloadCacheDirectory(), "conda-forge"));
    }
    ...
    static applicationSupportDirectory(): string {
        const directory = path.join(app.getPath("appData"), this.getAppSupportDirectoryName());
    ...
    static pythonWorkingDirectory(): string {
        return this.ensureDirectory(app.getPath("userData"));
    }
    ...
    static makePythonEnvironmentForRuntime(runtimeRoot: string): NodeJS.ProcessEnv {
        ...
        if (process.platform === "darwin") {
            ...
            const libraryPathEntries = [
                this.dyldLibraryPathForRuntime(runtimeRoot),
                path.join(runtimeRoot, "lib"),
            ];
            environment.DYLD_LIBRARY_PATH = libraryPathEntries
                .concat(environment.DYLD_LIBRARY_PATH ? [environment.DYLD_LIBRARY_PATH] : [])
                .join(path.delimiter);
        ...
        environment.LABEL_STUDIO_RUNTIME_CACHE = this.pipCacheDirectory();
        ...
        if (process.platform === "win32") {
            pathEntries.push(
                ...
                path.join(runtimeRoot, "Library", "usr", "bin"),
            );
    ...
    static makeRuntimeManagerEnvironment(): NodeJS.ProcessEnv {
        ...
        if (this.isExecutable(runtimePython)) {
            // When running manage_runtime.py with the embedded runtime Python, it needs the
            ...
            // When no embedded runtime exists yet, manage_runtime.py must run with the

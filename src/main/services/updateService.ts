...
export class UpdateService {
    ...
    private readonly activeRuntimeCommands = new Set<Promise<unknown>>();
    ...
    async waitForRuntimeReadersToSettle(): Promise<void> {
        while (this.versionFetchPromise || this.activeRuntimeCommands.size > 0) {
            const pending = [
                ...(this.versionFetchPromise ? [this.versionFetchPromise] : []),
                ...this.activeRuntimeCommands,
            ];
    ...
    async fetchVersions(options: VersionFetchOptions = {}): Promise<RuntimeVersions> {
        if (options.force && this.versionFetchPromise) {
            await this.versionFetchPromise.catch(() => undefined);
        }
    ...
    private async run<T>(command: string, timeout?: number): Promise<T> {
        const operation = this.runRuntimeCommand<T>(command, timeout);
        this.activeRuntimeCommands.add(operation);
        try {
            return await operation;
        } finally {
            this.activeRuntimeCommands.delete(operation);
        }
    }

    private async runRuntimeCommand<T>(command: string, timeout?: number): Promise<T> {
        const pythonURL = AppPaths.runtimePython();

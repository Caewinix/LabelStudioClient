{
    ...
    let downloadControlsInteractive = false;
    ...
    function reportRendererReadyAfterLayout(): void {
        ...
        try {
            ...
            console.error('Splash renderer layout failed before first show:', error);
        }
    ...
    function layout(): void {
        ...
        const detailWidth = 700 * scale;
        const detailMinimumHeight = 96 * scale;
        detail.style.width = px(detailWidth);
        detail.style.height = 'auto';
        detail.style.minHeight = px(detailMinimumHeight);
        detail.style.fontSize = px(31.5 * scale);
        const detailHeight = Math.max(detailMinimumHeight, Math.ceil(detail.scrollHeight));
        ...
        const textFrames: Frame[] = [
            ...
            { x: 0, y: detailTopBase, width: detailWidth, height: detailHeight }
        ];
        const downloadLabelHeight = 36 * scale;
        ...
        setTopLeftFrame(detail, 0, contentTop + detailTopBase, detailWidth, detailHeight, cardBoundsHeight);
        ...
        download.style.lineHeight = px(34 * scale);
        ...
        downloadPauseButton.style.pointerEvents = downloadControlsInteractive ? 'auto' : 'none';
        downloadCancelButton.style.pointerEvents = downloadControlsInteractive ? 'auto' : 'none';
        downloadPauseButton.disabled = !downloadControlsInteractive;
        downloadCancelButton.disabled = !downloadControlsInteractive;
    ...
    function stageShouldKeepDownloadUI(stage: LaunchStage): boolean {
        return stage.showsDownloadProgress === true;
    }
    ...
    function applyStage(stage: LaunchStage): void {
        ...
        downloadControlsInteractive = nextShowsDownloadProgress;
        ...
        if (nextShowsDownloadProgress) {
            ...
            downloadControlsInteractive = false;
        }
    ...
    function downloadButtonIsInteractive(button: HTMLButtonElement): boolean {
        return downloadControlsInteractive && !button.classList.contains('hidden') && !button.disabled;
    }

    async function triggerPauseToggle(event?: Event): Promise<void> {
        event?.preventDefault();
        event?.stopPropagation();
        if (!downloadButtonIsInteractive(downloadPauseButton)) return;
        if (pauseToggleInFlight) return;
        if (cancelInFlight || guardRepeatedAction(lastPauseActionAt, 140)) return;
        lastPauseActionAt = Date.now();
        pauseToggleInFlight = true;
        if (!isMouseInteraction(event)) flashButton(downloadPauseButton);

        const optimisticPaused = !isDownloadPaused;
        isDownloadPaused = optimisticPaused;
        updateDownloadControlImages();
        layout();

        try {
            if (typeof window.splashAPI?.togglePause === 'function') {
                const paused = await window.splashAPI.togglePause();
                if (typeof paused === 'boolean') {
                    isDownloadPaused = paused;
                    updateDownloadControlImages();
                    layout();
                }
            } else if (typeof window.splashAPI?.setPaused === 'function') {
                window.splashAPI.setPaused(optimisticPaused);
            }
        } catch (error) {
    ...
    async function triggerCancelDownload(event?: Event): Promise<void> {
        ...
        cancelInFlight = true;
        if (!isMouseInteraction(event)) flashButton(downloadCancelButton);

        // Pause immediately while main presents the cancellation confirmation.
    ...
    function bindExactDownloadButton(button: HTMLButtonElement, handler: (event: Event) => void | Promise<void>): void {
        ...
        const buttonIsInteractive = (): boolean => {
            return button.isConnected && downloadButtonIsInteractive(button);
        };
        ...
        button.addEventListener('click', (event) => {
            ...
            if (buttonIsInteractive()) void handler(event);
        }, { passive: false });
    ...
    try {
        ...
        console.error('Splash renderer initialization failed:', error);
    }
